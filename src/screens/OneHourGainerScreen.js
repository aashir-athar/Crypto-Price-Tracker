...
const OneHourGainerScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(fetchData, 3600000); // Fetch 1 hour 
  ...
  useEffect(() => {
    ...
    if (previousData) {
      ...
      setSortedData(updatedData);
        ...
        setSortedData(GmainData.mainData);
     }
  }, [GmainData.mainData]);
      ...
      <CustomCard
        onPress={() =>
          navigation.navigate("Coin-Details", {
            ...
            low24h: item.low24h,
            change1h: item.change15s.toFixed(2),
          })
        }
  ...
  return (
    <Screen>
      <Text style={styles.heading}>Top Gainers (1H)</Text>
      {sortedData ? (
        <FlatList
          data={sortedData}
...
export default OneHourGainerScreen;
